...
void ReservationTable::insertPath2CT(const Path& path)
{
	...
	++curr;
	// print to check if curr is nullptr, if so, print the location of prev
	// cout << "prev: " << prev->location << endl;
...
// void ReservationTable::build(const vector<Path*>& paths,
//         const list< tuple<int, int, int> >& initial_constraints,
//         const unordered_set<int>& high_priority_agents, int current_agent, vector<State> start_locations)
// {
//     clock_t t = std::clock();
...
//     if (prioritize_start) // prioritize waits at start locations
//     {
//         insertConstraints4starts(paths, current_agent, start_locations);
//     }
...
void ReservationTable::insertConstraints4starts(const vector<Path*>& paths, int current_agent, vector<State> start_locations)
{
    for (int i = 0; i < num_of_agents; i++)
    {
        if (paths[i] == nullptr)
            // continue;
			ct[start_locations[i].location].emplace_back(0, start_locations[i].timestep + k_robust);
