...
bool SymboticGrid::load_map(std::string fname)
{
...
// bool SymboticGrid::load_weighted_map(std::string fname)
// {
// 	std::string line;
...
// load map
bool SymboticGrid::load_unweighted_map(std::string fname)
{
    std::string line;
    std::ifstream myfile ((fname).c_str());
	if (!myfile.is_open())
	...
	for (int i = 0; i < rows; i++)
	{
		...
		for (int j = 0; j < cols; j++)
		{
			...
			else if (line[j] == 'i') //endpoint
			{
				types[id] = "Inbound";
				weights[id][4] = 1;
				inbounds.push_back(id);
				agent_home_locations.push_back(id);

			}
			else if (line[j] == 'o') //robot rest
			{
				types[id] = "Outbound";
				weights[id][4] = 1;
				outbounds.push_back(id);
				agent_home_locations.push_back(id);
    ...
    // std::cout << "Map size: " << rows << "x" << cols << " with ";
	// cout << endpoints.size() << " endpoints and " <<
	// cout << agent_home_locations.size() << " home stations." << std::endl;		
    // std::cout << "Done! (" << runtime << " s)" << std::endl;
    return true;
}

void SymboticGrid::preprocessing(bool consider_rotation)
{
	...
	if (!succ)
	{
		...
		{
			heuristics[inbound] = compute_heuristics(inbound);
		}
		for (auto outbound : outbounds)
		{
			heuristics[outbound] = compute_heuristics(outbound);
		}
        for (auto home : agent_home_locations)
		{
			heuristics[home] = compute_heuristics(home);
		}
        for (auto asile : asiles)
		{
			heuristics[asile] = compute_heuristics(asile);
		}
