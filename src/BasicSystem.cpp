...
bool BasicSystem::load_locations()
{
    ...
    {
        throw std::runtime_error("BasicSystem::load_locations: The agent file does not match the settings.");
    }
...
int BasicSystem::find_num_wait_agent_solution(const vector<Path>& paths) const
{
    int num_wait_agents = 0;
    for (const auto& path : paths)
    {
        int t = 0;
        int start_timestep = 0;
        while (t < simulation_window && path[start_timestep].location == path[start_timestep + t].location &&
                path[start_timestep].orientation == path[start_timestep + t].orientation)
            t++;
        // if (t == simulation_window)
        if (t == simulation_window)
            num_wait_agents++;
    ...
    // Debug output
    // std::cout << "DEBUG: num_wait_agents = " << num_wait_agents << ", num_of_drives = " << num_of_drives 
    //           << ", ratio = " << (double)num_wait_agents / num_of_drives << std::endl;
    
    return num_wait_agents;  // more than half of drives didn't make progress
}

// int BasicSystem::find_num_wait_agent_solution() const
// {
//     if (simulation_window <= 1)
//         return 0;
//     int wait_agents = 0;
            ...
            if (t > 0)
            {
                ...
                if (curr.location == prev.location)
                {
                    if (G.get_rotate_degree(prev.orientation, curr.orientation) == 2)
                    {
                        ...
                        save_results();
                        throw std::runtime_error("BasicSystem::move: 180-degree rotation detected");
                    }
                }
                else if (consider_rotation)
                {
                    if (prev.orientation != curr.orientation)
					{
						...
						save_results();
						throw std::runtime_error("BasicSystem::move: rotation while moving detected");
					}
                        ...
                        prev.location + G.move[prev.orientation] != curr.location)
                    {
                        ...
                        save_results();
                        throw std::runtime_error("BasicSystem::move: invalid jump move (rotation)");
                    }
                }
				else
				{
					...
					if (dir < 0 || !G.valid_move(prev.location, dir))
					{
						cout << "Drive " << k << " jump from " << prev << " to " << curr << endl;
						save_results();
						throw std::runtime_error("BasicSystem::move: invalid jump move");
					}
				}
            }
				...
				for (int j = k + 1; j < num_of_drives; j++)
				{
					for (int i = max(0, t - k_robust); i <= min(t + k_robust, end_timestep); i++)
					{
						...
						if (paths[j][i].location == curr.location)
						{
							...
							throw std::runtime_error("BasicSystem::move: vertex conflict detected");
						}
					}
				}
			}
...
void BasicSystem::save_results()
{
    ...
    output << "map: " << G.map_name << std::endl
        ...
        << "low_level_planning_window: " << low_level_planning_window << std::endl
...
int BasicSystem::find_dis_to_goal_after_executed(const vector<Path>& paths) const
{
    int avg_dis_to_goal = 0;
    int timestep = this->simulation_window;
    for (int k = 0; k < num_of_drives; k++)
    {
        int dis_to_goal = 0;
        if (paths[k].empty())
        {
            std::cout << "Path " << k << " is empty." << std::endl;
            return 0;
        }
        int location_after_executed = paths[k].at(timestep).location;
        if (goal_locations[k].empty())
        {
            int goal = location_after_executed;
            std::cout << "Goal location " << k << " is empty." << std::endl;
        }
        else
        {
            for (int i=0; i<goal_locations[k].size(); i++)
            {
                int goal = goal_locations[k][i].first;
                dis_to_goal += G.get_Manhattan_distance(location_after_executed, goal);
            }
            dis_to_goal /= goal_locations[k].size();
        }
        avg_dis_to_goal += dis_to_goal;
        
    }
    return avg_dis_to_goal;
}

int BasicSystem::find_num_confilicts_solution(const vector<Path>& paths) const
{
    int num_conflicts = 0;
    for (int a1=0; a1<num_of_drives; a1++)
    {
        for (int a2=a1+1; a2<num_of_drives; a2++)
        {
            ...
            {
                std::cout << "Path " << a1 << " or " << a2 << " is empty." << std::endl;
                return 0;
            }

            int size1 = paths[a1].size();
            int size2 = paths[a2].size();
...
void BasicSystem::solve()
{
	 ...
	 else // PBS or ECBS
    {
            ...
            if (sol)
                {
                    ...
                    {
                        throw std::runtime_error("BasicSystem::solve: COLLISIONS detected in planned_paths");
                    }
                }
                else
                {
                    ...
                    if (!sol) {
                        throw std::runtime_error("BasicSystem::solve: WHCA fallback failed to find a solution");
                    }
                }
            }
        ...
        else
        {
            ...
            solve_time = (end - start) / CLOCKS_PER_SEC;
            
            // std::cout << "DEBUG: solver.run() result = " << (sol ? "SUCCESS" : "FAILED") << std::endl;
            ...
            if (sol)
            {
                ...
                for (int k = 0; k < num_of_drives; k++)
                {
                    avg_solution_length += solver.solution[k].size();
                }
                avg_solution_length /= num_of_drives;

                avg_num_collision_solver = find_num_confilicts_solution(solver.solution)/num_of_drives;

                avg_dis_to_goal_after_excuted = find_dis_to_goal_after_executed(solver.solution)/num_of_drives;
                
                ratio_wait_agent_solver = find_num_wait_agent_solution(solver.solution)/num_of_drives;
                //  ratio_wait_agent_solver = find_num_wait_agent_solution()/num_of_drives;
            ...
            {  
                ...
                // Calculate waiting ratio from the original solver solution BEFORE LRA repair
                // int wait_count = find_num_wait_agent_solution(solver.solution);
                // ratio_wait_agent_solver = (double)wait_count / num_of_drives;
                ...
                for (int k = 0; k < num_of_drives; k++)
                {
                    avg_solution_length += lra.solution[k].size();
                }
                avg_solution_length /= num_of_drives;
                avg_num_collision_solver = find_num_confilicts_solution(lra.solution)/num_of_drives;

                avg_dis_to_goal_after_excuted = find_dis_to_goal_after_executed(lra.solution)/num_of_drives;
                ratio_wait_agent_solver = find_num_wait_agent_solution(lra.solution)/num_of_drives;
                // ratio_wait_agent_solver = find_num_wait_agent_solution()/num_of_drives;
                // std::cout << "DEBUG: LRA branch taken - solver failed, using LRA solution for execution but original solver for waiting ratio" << std::endl;
            }
...
void BasicSystem::initialize_solvers()
{
    ...
    solver.low_level_window = low_level_planning_window;
...
bool BasicSystem::load_records()
{
	...
	if (atoi(line.c_str()) != num_of_drives)
	{
		throw std::runtime_error("BasicSystem::load_records: The path file does not match the settings.");
	}
	for (int k = 0; k < num_of_drives; k++)
	{
		getline(myfile, line);
	...
	if (atoi(line.c_str()) != num_of_drives)
	{
		throw std::runtime_error("BasicSystem::load_records: The task file does not match the settings.");
	}
	for (int k = 0; k < num_of_drives; k++)
	{
		getline(myfile, line);
