...
void PBS::generate_root_node(PBSNode* init_node)
{
    dummy_start = init_node;
	...
	while (!dfs.empty() && !solution_found)
	{
        ...
        if (!solution_found)
        {
            if (n[0] != nullptr && n[1] != nullptr)
            {
                ...
                // if (n[0]->num_of_collisions < n[1]->num_of_collisions)
                {
                    push_node(n[1]);
                    push_node(n[0]);
                }
...
void PBS::assign_initial_paths(const PBSNode* curr, const std::vector<Path*>& look_ahead_path_copy, PBS* look_ahead_solver) {
    look_ahead_solver->initial_paths.resize(num_of_agents);

    for (size_t i = 0; i < look_ahead_path_copy.size(); ++i) {
        if (look_ahead_path_copy[i] != nullptr) {
            look_ahead_solver->initial_paths[i] = *look_ahead_path_copy[i]; // Dereference the pointer to get the Path object
        }
    }
    ...
    for (auto p = curr->paths.begin(); p != curr->paths.end(); ++p)
        ...
        look_ahead_solver->initial_paths[std::get<0>(*p)] = std::get<1>(*p);
