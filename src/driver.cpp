...
void set_parameters(BasicSystem& system, const boost::program_options::variables_map& vm)
{
	...
	system.time_limit = vm["cutoffTime"].as<double>();
	system.simulation_window = vm["simulation_window"].as<int>();
	system.planning_window = vm["planning_window"].as<int>();
	system.low_level_planning_window = vm["low_level_planning_window"].as<int>();
...
MAPFSolver* set_solver(const BasicGraph& G, const boost::program_options::variables_map& vm)
{
	...
	else if (solver_name == "PBS")
	{
		...
		pbs->use_look_ahead = vm["use_look_ahead"].as<bool>();
...
int main(int argc, char** argv) 
{
	...
	desc.add_options()
		...
		("scenario", po::value<std::string>()->required(), "scenario (KIVA, SYMBOTIC)")
		...
		("low_level_planning_window", po::value<int>()->default_value(INT_MAX / 2),
		        "planning window used in low level solver")
		("use_look_ahead", po::value<bool>()->default_value(false), "for PBS only, wheather to use antoher look ahead PBS at each node in PT")
	...
	else if (vm["scenario"].as<string>() == "SYMBOTIC")
	{
		SymboticGrid G;
		if (!G.load_map(vm["map"].as<std::string>()))
			return -1;
		MAPFSolver* solver = set_solver(G, vm);
		SymboticSystem system(G, *solver);
