...
public:
    ...
    // double run(const vector<State>& starts,
    //         const vector< vector<pair<int, int> > >& goal_locations, // an ordered list of pairs of <location, release time>
    //         double time_limit, PBSNode* init_node);
...
protected:
   ...
   //  vector<State> starts;
    ...
    void generate_root_node(PBSNode* init_node);
    ...
    void assign_initial_paths(const PBSNode* curr, const std::vector<Path*>& look_ahead_path_copy, PBS* look_ahead_solver);
