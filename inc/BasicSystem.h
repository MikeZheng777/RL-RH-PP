...
public:
    ...
    int num_new_finished_task;
    bool solver_success_flag = false;
    double avg_solution_length = -1000; 
    ...
    std::vector<std::list<std::pair<int, int> > > finished_tasks;
    // std::vector<Path> shortest_paths; // location + finish time
    ...
    int find_num_confilicts_solution(const vector<Path>& paths) const;
    int find_dis_to_goal_after_executed(const vector<Path>& paths) const;
    int find_num_wait_agent_solution(const vector<Path>& paths) const;
    // int find_num_wait_agent_solution() const;
    double avg_dis_to_goal_after_excuted = 0;
    double avg_num_collision_solver = 0;
    double ratio_wait_agent_solver = 0;
